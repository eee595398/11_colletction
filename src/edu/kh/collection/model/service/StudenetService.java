...
import edu.kh.collection.model.svo.Student;
...
public class StudenetService {
	...
	// ==Student 만 저장 가능 == 모든게 Student
	// == Student임을 검사할 필요가 없다.
	
	public StudenetService() {
		studentList.add(new Student("홍길동", 25, "서울시 중구", 'M', 90) );
		studentList.add(new Student("김길동", 25, "서울시 중구", 'M', 90) );
		studentList.add(new Student("이길동", 25, "서울시 중구", 'M', 90) );
		studentList.add(new Student("박길동", 25, "서울시 중구", 'M', 90) );
		studentList.add(new Student("미길동", 25, "서울시 중구", 'M', 90) );
	...
	public void ex() {
		...
		//studentList.add(new Student()); // 0번 인엑스
		//studentList.add(sc);
		//studentList.add("문자열");
		//studentList.add(new Object());
		// 컬랙션 특징 : 여러 타입의 데이터를 저장 할 수 있다.
	...
	public void displayMenu() {
		...
		do {
			...
			try {
				...
				// menuNum이 0을 가지고 있어 종료되는데 이를 방지하기 위해 임의 값 -1 대입
	...
	public String addStudent() throws InputMismatchException{
		...
		sc.nextLine();
		// 입력 버퍼 개행 문자제거 
		// 숫자 다음에 문자열을 입력하면 엔터를 문자열로 인식해서 오류 
		...
		if(studentList.add( new Student(name, age, region, gender, score))) {
			// boolean jave.util.List.add(Student e)
			// (반환형) 				   -> 제네릭 <Student>때문에 List 타입이 Student 제한됨 
	...
	public void selectAll() { 
		...
		//studentList가 비어 있는 경우 " 학생정보가 없습니다" 출력
		...
		for(Student std : studentList) {
			...
			//std에는 for문 반복시 마다 0,1,2..인덱스 요소들 한번씩 저장됨
	...
	public String updateStudent() throws InputMismatchException {
		...
		// -> 반환 값 Student == 변경 전 Student 객체가 담겨있다 
		...
		// 4) 입력받은 숫자가 studentList 범위 내 인덱스 번호인가?
			...
			// 이때, index 번째 에 있던 기본 학생 정보가 반환된다
	...
	public String removeStudent() throws InputMismatchException{
		...
		char ch = sc.next().toUpperCase().charAt(0); 
				// String 대문자 -> 대문자 0번 인덱스 문자 소문자를 -> 대문자로 변환 
